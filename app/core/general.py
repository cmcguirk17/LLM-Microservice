...
def load_config_from_yaml(
    ...
    try:
        ...
        if raw_config_data is None:
            raw_config_data = {}
            logger.warning(f"Configuration file '{config_path}' is empty or contains only comments. Proceeding with empty config data.")

        logger.info(f"Attempting to load configuration from '{config_path}'.")
        config_model = ClientConfig(**raw_config_data) # valErr if we don't match pydantic model
