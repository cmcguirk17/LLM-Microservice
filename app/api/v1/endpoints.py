...
async def health_check_status(request: Request) -> dict:
    ...
    llm_instance_available = hasattr(request.app.state, "llm") and request.app.state.llm is not None
...
async def create_chat_completion(
    ...
    """
    logger.info(f"Received chat completion request with {len(request_data.messages)} messages.")

    if not request_data.messages: # Basic validation
        logger.warning("Received empty messages list for chat completion.")
        raise HTTPException(status_code=400, detail="Messages list cannot be empty.")

    logger.debug(f"Chat completion request details: {request_data.model_dump_json(indent=2)}")
