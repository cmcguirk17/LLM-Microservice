...
def test_read_root():
    ...
    assert response.json() == {"message": f"Welcome to {APP_TITLE}. Visit /docs for API details."}


@pytest.mark.asyncio # Mark as an async test if using async def with pytest-asyncio
async def test_lifespan_model_loads_successfully(mocker): # mocker is from pytest-mock
    """
    ...
    # Mock the Llama class constructor and instance
    mock_llama_instance = MagicMock()
    mock_Llama_class = mocker.patch("app.main.Llama", return_value=mock_llama_instance) # Patch Llama in app.main

    # Mock time.time_ns for predictable load time calculation
    mocker.patch("time.time_ns", side_effect=[0, 1_000_000_000]) # Start and end time (1 second)

    # Create a dummy FastAPI app instance for the lifespan context
    test_app = FastAPI()

    # Manually enter and exit the async context manager
    ...
    # In your lifespan, you do `del app.state.llm`, so we check if it's gone.
...
async def test_lifespan_model_file_not_found(mocker):
    ...
    # Mock the Llama class (it should NOT be called if model path doesn't exist)
...
async def test_lifespan_model_load_fails_exception(mocker):
    """
    Test the lifespan when Llama class raises an exception during initialization.
    """

    mocker.patch("os.path.exists", return_value=True) # Model file "exists"
    mock_Llama_class = mocker.patch("app.main.Llama", side_effect=Exception("Test LLM Load Error")) # Llama init fails
